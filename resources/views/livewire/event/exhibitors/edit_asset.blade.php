                ...
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    ...
                    <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                        <button type="button"
                            ...
                            wire:click.prevent="editExhibitorAssetConfirmation">Update</button>
                        <button type="button"
                            ...
                            wire:click.prevent="cancelEditExhibitorAsset">Cancel</button>

                        @if ($assetType == 'Exhibitor banner')
                            @if (!$exhibitorData['exhibitorBannerDefault'])
                                <button type="button"
                                    class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-red-600 text-base font-medium text-white hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 sm:ml-3 sm:w-auto sm:text-sm"
                                    wire:click.prevent="removeExhibitorAssetConfirmation">Remove</button>
                            @endif
                        @else
                            @if (!$exhibitorData['exhibitorLogoDefault'])
                                <button type="button"
                                    class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-red-600 text-base font-medium text-white hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 sm:ml-3 sm:w-auto sm:text-sm"
                                    wire:click.prevent="removeExhibitorAssetConfirmation">Remove</button>
                            @endif
